...
export const FundraisersCarousel: React.FC<FundraisersCarouselProps> = ({ 
  fundraisers: propFundraisers, 
  title = "Personal Fundraisers" 
}) => {
  ...
  const fundraisers = propFundraisers || defaultFundraisers;
          ...
          <Card key={fundraiser.id} className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300">
            <CardHeader>
              <div className="flex items-center gap-3 mb-3">
                <Avatar>
                  <AvatarFallback>
                    {(fundraiser.organizer || fundraiser.charity || 'U').charAt(0)}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <CardTitle className="text-lg">{fundraiser.title}</CardTitle>
                  <p className="text-sm text-gray-600">
                    by {fundraiser.organizer || fundraiser.charity}
                  </p>
                </div>
              </div>
