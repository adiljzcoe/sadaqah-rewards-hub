...
interface CharityTin3DProps {
  isActive: boolean;
}

function CharityTin({ isActive }: CharityTin3DProps) {
  ...
  useFrame((state) => {
    if (tinRef.current) {
      if (!isActive) {
        ...
        tinRef.current.position.y = Math.sin(state.clock.elapsedTime * 2) * 0.05;
        tinRef.current.rotation.y = Math.sin(state.clock.elapsedTime * 0.5) * 0.02;
      } else {
        // Active shaking animation like a real collection tin
        tinRef.current.position.x = Math.sin(state.clock.elapsedTime * 25) * 0.1;
        tinRef.current.position.y = Math.cos(state.clock.elapsedTime * 30) * 0.08;
        tinRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 28) * 0.15;
      }
    }
  });
